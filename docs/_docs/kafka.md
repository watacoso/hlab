...
Kafka consumer and producers docker images used for testing ca be found in [kafka-python-client repository](https://github.com/watacoso/kafka-python-client). This docker image contain source code of one of the examples in [confluent-kafka-python repository]https://github.com/confluentinc/confluent-kafka-python/.
...
- Step 1: launch producer
  ```shell
  kubectl -n kafka run kafka-producer -ti --image=watacoso/kafka-python-client:latest --rm=true --restart=Never -- python avro_producer.py -b my-cluster-kafka-bootstrap:9092 -s http://kafka-schema-registry:8081 -t my-avro-topic
  ```
...
- Step 3: in a different terminal launch consumer
  ```shell
  kubectl -n kafka run kafka-consumer -ti --image=watacoso/kafka-python-client:latest --rm=true --restart=Never -- python avro_consumer.py -b my-cluster-kafka-bootstrap:9092 -s http://kafka-schema-registry:8081 -t my-avro-topic
  ```
...
Even when helm chart source code is available in the repository, this helm chart is not hosted in any official helm repository. I have decided to selfhost this helm chart within my own repository `https://watacoso.github.io/helm-charts/`
...
- Step 1: Add the Helm repository:
  ...
  ```shell
  helm repo add watacoso https://watacoso.github.io/helm-charts/
  ```
...
- Step 3: Prepare kafdrop-values.yml
  ...
  ingress:
    ...
    # ingress host
    hosts:
      - kafdrop.picluster.watacoso.com
    ## TLS Secret Name
    tls:
    ...
    annotations:
      ...
      cert-manager.io/common-name: kafdrop.picluster.watacoso.com
...
- Step 4: Install Kafdrop helm chart
  ...
  ```shell
  helm upgrade -i kafdrop watacoso/kafdrop -f kafdrop-values.yml --namespace kafka
  ```
- Step 4: Confirm that the deployment succeeded, opening UI:

  https://kafdrop.picluster.watacoso.com/
